...
function displaySyllabus(year, fac) {
    'use strict';
    if (
        location.href.includes('scombz.shibaura-it.ac.jp/lms/course?idnumber=')
    ) {
        ...
        if ($courseTitle) {
            ...
            */
            ...
            $courseTitle.parentNode.insertAdjacentHTML(
                'beforeEnd',
                `<a href="http://syllabus.sic.shibaura-it.ac.jp/namazu/namazu.cgi?ajaxmode=true&query=${$courseNameStrEnc}&whence=0&idxname=` +
                    year +
                    `%2F` +
                    fac +
...
function displaySyllabusError() {
    ...
    if (
        ...
        $courseTitle.parentNode.insertAdjacentHTML(
            'beforeEnd',
            `<span style="color:red;">シラバス表示をするには、<a href="javascript:void(0);" id="link_to_extention_syll">拡張機能設定</a>から、学年と学部を設定してください。</span>`
        );
        document
            .getElementById('link_to_extention_syll')
