...
function syllabusLoaded($settings_year, $settings_fac) {
    ...
    if (
        ...
        if (document.getElementsByTagName('dt')[15]) {
            ...
            if ($namazuHeader) {
                ...
                if ($score1word - $score2word < 6) {
                    ...
                    if (
                        $dltag.includes(
                            `<dt><strong class="keyword">1</strong>`
                        )
                    ) {
                        let $buggingArea = $dltag.slice(0, $score1 - 7);
                        $buggingArea = $buggingArea.slice(
                            $buggingArea.indexOf('href')
                        );
                        $buggingArea =
                            `<dt>1.<strong><a id="hit_1" ` + $buggingArea;
                        document.getElementsByTagName('dl')[2].innerHTML =
                            $buggingArea + $dltag.slice($score1 - 7);
                        console.log('置換します');
                    }
                    document
                        .getElementsByTagName('dl')[2]
                        .classList.add('result-list-box');
                    $namazuHeader.setAttribute('id', 'searchResult');
                    document
                        .querySelector('.namazu-result-footer')
                        .insertAdjacentHTML(
                            'afterEnd',
                            `
                        ...
                        getDepResponse()
                            .then((data) => {
                                ...
                                document
                                    ...
                                    [12 + cnt].insertAdjacentHTML(
                                        'afterBegin',
                                        `<div class="gakkaname-area">${gakkaIDtoStr(
                                            gakkaID
                                        )}</div>`
                                    );
                            })
        ...
        if ($sylSubjLink2) {
            $suggestSubj +=
                '?suggesting=true&sug1l=' +
                $sylSubjLink2.href.substring($sylSubjLink2.href.length - 14);
        }
        if ($autoredirect) {
            if ($sylSubjLink) {
            ...
            $.ajax({
                ...
                function (data) {
                    ...
                    document.body.insertAdjacentHTML(
                        `afterBegin`,
                        `<p style="margin-top:50px;">こちらの教科をお探しですか？</p>${$suggest}
                        `
                    );
                },
                function () {
